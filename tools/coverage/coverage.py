...
def launchCoverage(cdmath_build,src_cdmath,step,module_cdmath,coverage):
    ...
    if step=="a":
        proc=subprocess.Popen("lcov --zerocounters --directory "+directory_object,shell=True,stdout=sys.stdout,stderr=sys.stderr)
        so,se=proc.communicate()
        proc=subprocess.Popen("lcov --capture --initial --directory "+directory_object+" --output-file="+module_cdmath+"_init_"+extention+".info",shell=True,stdout=sys.stdout,stderr=sys.stderr)
        so,se=proc.communicate()
        pass
    else:
        proc=subprocess.Popen("lcov --capture --directory "+directory_object+" --output-file="+module_cdmath+"_tests_"+extention+".info",shell=True,stdout=sys.stdout,stderr=sys.stderr)
        so,se=proc.communicate()
        proc=subprocess.Popen("lcov -a "+module_cdmath+"_init_"+extention+".info -a "+module_cdmath+"_tests_"+extention+".info -o "+module_cdmath+"_tests_all_"+extention+".info",shell=True,stdout=sys.stdout,stderr=sys.stderr)
        so,se=proc.communicate()
        proc=subprocess.Popen("lcov --remove "+module_cdmath+"_tests_all_"+extention+".info '/usr/include/c++/4.7/x86_64-linux-gnu/bits/*' '/usr/include/*' '/usr/include/c++/4.7/*' '"+fi1+"/*' '"+fi2+"/*' --output "+module_cdmath+"_"+extention+".info",shell=True,stdout=sys.stdout,stderr=sys.stderr)
        so,se=proc.communicate()
