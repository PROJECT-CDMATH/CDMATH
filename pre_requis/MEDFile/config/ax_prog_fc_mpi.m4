...
AC_DEFUN([__AX_PROG_FC_MPI], [
  ...
  if (test x"$_ax_prog_fc_mpi_mpi_wanted" = xyes && test x"$med_check_f90" = xyes) ; then
    if test -z "$FC" && test -z "$f90prog" && test -n "$MPIFC"; then
dnl       echo "------------------FC 1 -----------------------"
      FC="$MPIFC"
    elif  test -z "$FC" && test -n "$f90prog"; then
dnl       echo "------------------FC 1b -----------------------"
      FC="$f90prog"
    elif  test -z "$FC"; then
dnl       echo "------------------FC 1c -----------------------"
...
AC_DEFUN([_AX_PROG_FC_MPI], [
  AC_REQUIRE([__AX_PROG_FC_MPI],[__AX_PROG_FC_MPI([$1])])
#echo "---------FC 3-----------------------"
dnl  AC_PROG_FC
  AS_IF([test -n "${f90prog}"],
  [
    if test x"$med_check_f90" = xyes; then 
