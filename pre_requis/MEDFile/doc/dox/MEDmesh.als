...
"MEDmeshGridIndexCoordinateRdDetails=Cette routine permet la lecture des coordonnées des noeuds d'un maillage structuré selon un axe du repère des coordonnées.\
 Pour N axes dans le repère des coordonnées, les numéros des axes vont de 1 à N. Pour lire les coordonnées dun maillage structuré de type #MED_CURVILINEAR_GRID,\
...
"MEDmeshGridIndexCoordinateWrRem=Cette routine ne permet pas l'écriture des noeuds d'un maillage structuré #MED_STRUCTURED_MESH de type #MED_CURVILINEAR_GRID." \
\
...
"MEDmeshNodeCoordinateWrnogridRem=Cette routine ne permet pas l'écriture des coordonnées des noeuds des maillages structurés #MED_STRUCTURED_MESH quelqu'en soit le type #med_grid_type." \
\
...
"MEDmeshNodeCoordinateWrgridRem1=Cette routine est également utilisée pour écrire les noeuds des maillages structurés #MED_STRUCTURED_MESH de type #MED_CURVILINEAR_GRID. Pour les autres types de maillages structurés (#MED_CARTESIAN_GRID, #MED_POLAR_GRID) utiliser la routine \ref MEDmeshGridIndexCoordinateWr qui permet l'écriture des noeuds principaux selon les axes choisis." \
\
...
le second tableau <i>nodeindex</i> renvoie pour chaque face à la liste des noeuds qui la composent. \
...
\n \n \image html exemple_connectivite_nodale_polyedres.png \n \n \
...
\li La définition de la connectivité descendante se base sur la numérotation locale à chaque type géométrique (exemple : 1..nq pour les #MED_QUAD4, 1..nt pour les #MED_TRIA3...). \
\n \n \image html exemple_connectivite_descendante_polyedres.png \n \n \
...
Le profil est identifié par un nom et le mode de stockage des données en mémoire peut être paramétré : #MED_COMPACT_PFLMODE ou #MED_GLOBAL_PFLMODE .\
 L'indicateur \a changement indique un changement dans le maillage par rapport à la séquence de calcul précédente (exemple : nouvelles coordonnées des noeuds).\
...
"MEDmeshEntityAttributeAdvancedRdBrief=Cette routine permet de lire les attributs optionnels d'entités d'un maillage en utilisant un filtre." \
"MEDmeshEntityAttributeAdvancedRdDetails=Cette routine permet de lire les attributs optionnels (noms,numéros,numéros de famille) d'entités d'un maillage en utilisant un filtre. " \
"MEDmeshEntityAttributeAdvancedRdRem=Le type d'attribut optionnel concerné est indiqué par le paramètre datatype qui peut prendre les valeurs suivantes : \n \
\li #MED_NAME : Noms optionnels \
\li #MED_NUM : Numéros optionnels \
